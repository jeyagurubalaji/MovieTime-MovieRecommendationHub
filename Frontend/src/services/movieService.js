...
export const movieService = {
  ...
  nowPlaying: (page = 1) =>
    api.get('/movies/now-playing', { params: { page } }).then((r) => r.data),
  upcoming: (page = 1) =>
  ...
  details: (id, type = 'movie') =>
    api.get(`/movies/${id}`, { params: { type } }).then((r) => r.data),
  watchProviders: (id, type = 'movie') =>
    api.get(`/movies/${id}/watch/providers`, { params: { type } }).then((r) => r.data),
  ...
  credits: (id) =>
    api.get(`/movies/${id}/credits`).then((r) => r.data),
  personDetails: (id) =>
    api.get(`/people/${id}`).then((r) => r.data),
  filter: (params) =>
    api.get('/movies/filter', { params }).then((r) => r.data),

  // Dynamically fetch Oscar-winning movies or TV shows via backend API
  oscarWinners: (category = 'Best Picture', page = 1) =>
    api.get('/movies/oscars', { params: { category, page } }).then((r) => r.data),

  // Dynamically fetch Oscar-winning actor or actress filmography
  personOscarCredits: (personId) =>
    api.get(`/people/${personId}/oscar-credits`).then((r) => r.data),
}

...
export default function LibraryActions({ movie }) {
  ...
  const mediaType = searchParams.get('type') || 'movie' // Extract type from the current URL
  ...
  const toggleFavorite = async () => {
    if (status.isFavorite) {
      ...
      await libraryService.addFavorite(movie, mediaType)
  ...
  const toggleWatchlist = async () => {
    if (status.isInWatchlist) {
      ...
      await libraryService.addToWatchlist(movie, mediaType)
  ...
  const toggleHidden = async () => {
    if (status.isHidden) {
      ...
      await libraryService.hide(movie, mediaType)
      setStatus((s) => ({ ...s, isHidden: true }))
    }
  }

  const confirmWatched = async () => {
    await libraryService.markWatched(movie, rating, mediaType)

...
// Import your push notifications service if available:
// import { initPushNotifications } from '../services/notificationService'

// Fallback definition to prevent runtime ReferenceErrors if notification setup is optional/pending
const initPushNotifications = window.initPushNotifications || (() => Promise.resolve())
...
export function AuthProvider({ children }) {
  ...
  const persistSession = ({ token, user: newUser }) => {
    ...
    // Safely attempt push notification initialization
    if (typeof initPushNotifications === 'function') {
      try {
        Promise.resolve(initPushNotifications()).catch(() => {})
      } catch {
        // Suppress notification initialization failures during login
      }
    }
  }

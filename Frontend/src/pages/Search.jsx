...
// STRICT FILTERS: Remove Talk Shows, News, Reality TV, and Documentaries
...
// SORT FIX: Keep popularity sort specifically for actors/directors so iconic roles show up first
...
function PersonResults({ people, jobFilter }) {
  ...
  const selectPerson = async (person) => {
    ...
    let credits = []
    if (jobFilter === 'Director') {
      credits = (details.combined_credits?.crew || []).filter(
        (c) => c.job === 'Director' || c.job === 'Creator' || c.job === 'Series Director' || c.job === 'Writer'
      )
    } else {
      credits = details.combined_credits?.cast || []
      if (credits.length === 0) {
        credits = details.combined_credits?.crew || []
      }
    }

    // STRICT FILTERS APPLIED HERE
    const filteredCredits = credits.filter(c => {
      // 1. Block excluded genres (Talk Shows, News, Reality, Docs)
      if (c.genre_ids && c.genre_ids.some(id => EXCLUDED_GENRES.includes(id))) {
        return false;
      }
      // 2. Block "Self" / "Himself" / "Guest" character appearances
      if (jobFilter !== 'Director') {
        const charName = (c.character || '').toLowerCase();
        if (charName.includes('self') || charName.includes('himself') || charName.includes('herself') || charName.includes('guest')) {
          return false;
        }
      }
      return true;
    ...
    const uniqueCredits = Array.from(new Map(filteredCredits.map((c) => [c.id, c])).values())
    const creditsWithPosters = uniqueCredits.filter(c => c.poster_path)

    setFilmography(creditsWithPosters.sort(sortByPopularity))
    setLoading(false)
  }
  ...
  return (
    <div>
      ...
      {selected && (
        <>
          <div className="section-heading"><h2>{selected.name}'s Movies & Shows</h2></div>

          {loading ? (
            <p className="muted">Loading filmography…</p>
          ) : filmography.length === 0 ? (
            <p className="muted">No movies or shows with posters found for this person.</p>
          ) : (
            <div className="movie-row">
              {filmography.map((m) => <MovieCard key={`${m.id}-${m.credit_id || m.job}`} movie={m} />)}
            </div>
          )}
        </>
      )}
...
export default function Search() {
  ...
  const [filters, setFilters] = useState({ sortBy: 'primary_release_date.desc' })
  ...
  const handleSearch = async (e) => {
    ...
    if (activeTab === 'title') {
      ...
      const moviesAndShowsOnly = (data.results || []).filter(item => item.media_type !== 'person' && item.poster_path)
      setResults(moviesAndShowsOnly)
    } else if (activeTab === 'actor' || activeTab === 'director') {
      const data = await movieService.searchPeople(query)
      const peopleWithPhotos = (data.results || []).filter(person => person.profile_path)
        ...
        {activeTab === 'ai' ? (
          ...
          <>
            ...
            {!loading && searched && (activeTab === 'actor' || activeTab === 'director') && people.length === 0 && (
              <p className="muted">No people with photos found for "{query}".</p>
            )}

...
export default function ReleaseCalendar() {
  ...
  useEffect(() => {
    setLoading(true)
    discoveryService.releaseCalendar(year, month)
      .then((data) => {
        // STRICT FILTER: Only keep releases that have an official poster
                  ...
                  {dateMovies.map((m) => (
                    <Link key={m.id} to={`/movie/${m.id}`} style={{ width: 100 }}>
                      <img
                        ...
                        alt={m.title}
                        style={{ width: '100%', borderRadius: 8, aspectRatio: '2/3', objectFit: 'cover' }}
                      />

...
# CHANGED: Added strict instructions to sanitize explicit/mature content to bypass LLM safety filters
...
beyond what's in the given synopsis. CRITICAL: If the original synopsis contains mature, explicit, violent, or sensitive themes, \
you MUST sanitize your summary to be safe, professional, and PG-13. \
...
async def generate_movie_summary(movie_id: int, media_type: str = "movie") -> dict:
    ...
    try:
        summary = await llm_client.complete(
            ...
            max_tokens=600, # INCREASED: Gives the AI plenty of room to finish the paragraph
        )

        # Clean the output
        summary_clean = summary.strip() if summary else ""

        # Catch empty responses or standard AI safety refusal phrases
        if not summary_clean or "I cannot fulfill" in summary_clean or "I apologize" in summary_clean:
            raise ValueError("LLM returned an empty response or safety refusal.")

        return {"movie_id": movie_id, "title": title, "summary": summary_clean, "ai_powered": True}
    except Exception as e:
        logger.warning("AI summary generation failed, falling back to original overview: %s", e)
        # Sets ai_powered to False, which hides the empty box on the frontend
